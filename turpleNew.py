...
'2||101300070316101018001CVS64VSGODFYUUWJRFD4B7EXGIZBK6BVHWKYY3JCUGDIF2UYW3RDO4PGI3RMMLQDCTNQHQMG6M2VKKTSQRXEQTDXOOVDMMNJ3NGWHDT6W6LE2UUBXBLJNTLQEWLHW2ARQ||01000000207610119000356142||01000000207620119000002633\n'
]
...
def listLenTuple_1_AllValuesAndKeys():
    for i in tuple_1:
        print('\n', i, ' - ', len(tuple_1[i]))
        for j in tuple_1[i]:
            print(' ____', j ,'___', len(tuple_1[i][j]))
...
def deleteCodesFromTuple_1(pal, box, code, aaa):
    aaa[pal][box].remove(code)
def deleteBoxesFromTuple_1(pal, box, aaa):
    if len(aaa[pal][box]) == 0:
        del aaa[pal][box]
def deletePalsFromTuple_1(pal, aaa):
    if len(aaa[pal]) == 0:
        del aaa[pal]

def listTuple_1_AllValuesAndKeysDeleteCodes():
    pals = []
    for pal2 in tuple_1:
        pals.append(pal2)
        boxes = []
        #print('\n', i, ' - ', len(tuple_1[i]))
        for box2 in tuple_1[pal2]:
            boxes.append(box2)
            #print(' ____', j ,'___', len(tuple_1[i][j]))
            codes = []
            for code2 in tuple_1[pal2][box2]:
                codes.append(code2)
            if codes:
                for code2 in codes:
                    deleteCodesFromTuple_1(pal2, box2, code2, tuple_1)
        if boxes:
            for box3 in boxes:
                if len(tuple_1[pal2][box3]) == 0:
                    deleteBoxesFromTuple_1(pal2, box3, tuple_1)
    if pals:
        for pal3 in pals:
            if len(tuple_1[pal2]) == 0:
                deletePalsFromTuple_1(pal3, tuple_1)
                #print('=========', k)

def findCodeInTuple_1(code, aaa):
    for pal in aaa:
        for box in aaa[pal]:
            if code in aaa[pal][box]:
                #print('\npal = ', pal, '\nbox = ', box, '\ncode = ', code)
                destinationOfCode = [pal, box, code]
                return destinationOfCode

def deleteCodesFromTuple_1(list11):
    for i in list11:
        inList11 = re.findall(r'\w+', i)
        fff = findCodeInTuple_1(inList11[1], tuple_1)
        if fff:
            #print('\ntuple_1 {0} {1} = {2}'.format(fff[0], fff[1], inList11[1]))
            #print('1 len = ', len(tuple_1[fff[0]][fff[1]]))
            tuple_1[fff[0]][fff[1]].remove(inList11[1])
            #print('\ntuple_1 {0} {1} = {2}'.format(fff[0], fff[1], tuple_1[fff[0]][fff[1]]))
            #print('2 len = ', len(tuple_1[fff[0]][fff[1]]))
